...
namespace AlphaRooms.AutomationFramework.Tests.Live_Tests.Hotel_Only_Top_Destinations_Tests
{
    ...
    public class HotelOnlyExtrasTests : AlpharoomsTestBase
    {
        ...
        public void ShouldSearch_MostPopularRoutes_ForAllMultiCom_Suppliers(HotelExtrasTestScript script)
        {
            ...
            Assert.That(ExtrasPage.IsExtraDisplayed(Extras.CarHire), "Car Hire is not displayed on extra page");

            //check if Airport Transfer is displayed
            //Assert.That(ExtrasPage.IsVisible(Extras.Airport_Transfer), "Airport Transport is not displayed on extra page");
            ...
            //Confirm if any results are available
            //Assert.That(ExtrasPage.CheckResultsAvailabilityFor(Extras.Airport_Transfer), "Airport Transfer results not available");

            //Assert.That(ExtrasPage.IsVisible(Extras.Airport_Transfer), "Airport Transfer is not Visible on extra page");

            //Assert.That(ExtrasPage.IsVisible(Extras.Hold_luggage), "Airport Transfer is not Visible on extra page");
