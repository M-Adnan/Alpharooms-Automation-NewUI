...
namespace AlphaRooms.AutomationFramework.Tests.Smoke_Tests
{
    ...
    public class HotelOnlyBookingTests : AlpharoomsTestBase
    {
        ...
        public void ShouldMakeHotelOnlyBooking()
        {
            ...
            Assert.That(HomePage.IsDisplayed(), "Homepage isn't displayed");
            ...
            Assert.That(ExtrasPage.IsDisplayed, "Extra page is not been displayed");

            //check if carhire is displayed
            Assert.That(ExtrasPage.IsExtraDisplayed(Extras.CarHire), "Car Hire is not Visible on extra page");

            //Expand Car Hire
            ExtrasPage.ExpandExtraLink(Extras.CarHire);

            //Confirm if any results are available
            Assert.That(ExtrasPage.AreResultsDisplayed(Extras.CarHire), "Car Hire results not available by default");
            ...
            Assert.That(HotelDetailPage.IsDisplayed(), "Hotel Result page is not displayed");

            //
            HotelDetailPage.SelectRoom().OnlyOneRoomWithAvailableRoom(1).ContinueAndCapture();

            //extra page is displayed
            Assert.That(ExtrasPage.IsDisplayed, "Hotel Summary page is not been displayed");


            //
            Assert.That(ExtrasPage.IsExtraDisplayed(Extras.CarHire),Is.True,"Car Hire is not Visible on extra page");

            Assert.That(ExtrasPage.IsExtraDisplayed(Extras.AirportTransfer), "Airport Transfer is not Visible on extra page");

            Assert.That(ExtrasPage.IsExtraDisplayed(Extras.HoldLuggage), "Airport Transfer is not Visible on extra page");
    ...
    //        Assert.That(HotelResultsPage.ContainsHotelByName("Viviendas Margarita II"), "No hotel available with this name");
    ...
    //        Assert.That(HotelDetailPage.IsDisplayed, "Hotel Summary page is not been displayed");
    ...
    //        Assert.That(HotelResultsPage.ContainsRoomFromSupplier("Hotels4U (19)"), "Supplier not found on current select page");
    ...
    //    //    Assert.That(ExtrasPage.Confirm(Information.Hotel), "Selected flight information doesn't match with flight information on Extras page");
    ...
    //    //    Assert.That(PaymentPage.Confirm(Information.Hotel), "Selected flight information doesn't match with flight information on Payment page");
